/*
* Для оптимихации использовал SIMD.(пока что без параллели)
* Использовал Векторизацию, т.е. INTEL AVX(была идея с SSE, но я не умею)
* Векторизация оптимизировала циклы, хотя в основной программе можно было просто изменить порядок и впринципе все, но решил так.
* наверное теперь по пунктам:
    1) relax
    использую функцию из <immintrin.h> для векторизации цикла, т.е. _mm256_loadu_pd(в отчете будет подробно описано, но сейчас как сноска)
    для чего? Что бы загружать по 4 элемента и с усреднением, и деление на 8(_mm256_div_pd(тоже опишу потом в отчете)), как было в основе
*
    2)resid
    уже тут немного игры для вычисления максимума в одно движение руки
*
    3)verify
    Умножаем уже A[i][j] на соотв. индексы и рез суммируем

* время не засекал, потом уже буду
* компиляция: (icc)\gcc SIMD_16_rewrite_AVX.c -o SIMD_1 -mavx -std=c99 -lm
* s = 42511
* -mavx для включения AVX инструкций
* Time: 0.01 sec
*  NxN: 4356
* N: 66
*/
